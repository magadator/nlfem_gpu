...
namespace KARMA
{
    namespace BuiltIns
    {
        ...
        std::string PTLFunc_fread(std::vector<std::string>& args)
        {
            ...
            if (idx >= fileContents.size()) throw BuiltinException("Provided index \"" + std::to_string(idx) + "\" out of range: found only " + std::to_string(fileContents.size()) + " lines (zero-base indexing!).");
